...
int main() {
  ...
  while (1) {
    ...
    case 1: {
      ...
      warn(&data);
      insert_front(data);
      printf("new node containing \'%d\' has been inserted at the front of the "
             "list.\n"
    ...
    case 2: {
      ...
      warn(&data_prev);
      if (!search(data_prev)) {
        printf("\ninvalid insertion!\n(node containing \'%d\' doesn't exist in "
               "the list)\n",
               data_prev);
        break;
      }
    ...
    case 3: {
      ...
      warn(&data);
      insert_rear(data);
      printf("new node containing \'%d\' has been inserted at the end of the "
             "list.\n"
             "(current number of node(s) in the list = %d)\n",
             data, list_size());
      break;
    ...
    case 4: {
      ...
      data = free_front();
      printf(
          "node containing \'%d\' has been freed from the front of the list.\n"
    ...
    case 7: {
      ...
      if ((index = search(data))) {
        if (index == 1)
          printf("the node containing \'%d\' is available at the very front of "
                 "the list.\n",
                 data);
        else
          printf("the node containing \'%d\' is available at the distance of "
                 "%d node(s) from head.\n",
                 data, index);
      } else
